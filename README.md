...
```
ronflexcorp/
├── index.html          → page principale (structure de toutes les sections)
├── css/
│   └── style.css       → tous les styles (palette, typographie, animations, responsive)
├── js/
│   └── main.js         → interactions : nav, scroll reveal, stats animées, formulaires,
│                          moteur du catalogue Pension (recherche, filtres, tri, pagination)
├── data/
...
- `types` : un tableau d'1 ou 2 types parmi ceux définis dans `TYPE_COLORS` (js/main.js)

Pour brancher une vraie API au lieu d'un fichier JSON statique, modifiez la fonction
`loadCatalog()` dans `js/main.js` (remplacez l'URL `data/pokemon.json` par l'endpoint de
votre backend).
